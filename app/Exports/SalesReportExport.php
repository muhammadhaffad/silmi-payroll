...
class SalesReportExport implements FromCollection, WithStyles, WithCustomStartCell
{
    protected $devision;
    protected $month;
    protected $year;
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->setCellValue('H9', "T. Operasional");
        $sheet->getColumnDimension('H')->setWidth(16);
        $sheet->setCellValue('I9', "T. Kepala Keluarga");
        $sheet->getColumnDimension('I')->setWidth(16);
        $sheet->setCellValue('J9', "T. Masa Kerja");
        $sheet->getColumnDimension('J')->setWidth(16);
        $sheet->setCellValue('K9', "T. Lain-lain");
        $sheet->getColumnDimension('K')->setWidth(16);
        $sheet->setCellValue('L9', "Lembur");
        $sheet->getColumnDimension('L')->setWidth(16);
        $sheet->setCellValue('M9', "Reward");
        $sheet->getColumnDimension('M')->setWidth(16);
        $sheet->setCellValue('N9', "Infaq");
        $sheet->getColumnDimension('N')->setWidth(16);
        $sheet->setCellValue('O9', "Cicilan");
        $sheet->getColumnDimension('O')->setWidth(16);
        $sheet->setCellValue('P8', "Take Home");
        $sheet->getColumnDimension('P')->setWidth(16);
        $sheet->mergeCells('P8:P9', Worksheet::MERGE_CELL_CONTENT_MERGE);
        $sheet->getStyle('A8:P9')->applyFromArray($styleArray);
        $sheet->getStyle('A8:P9')->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
        $sheet->getStyle('A8:P9')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
        $sheet->getStyle('A8:P9')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('DDDDDD');
        ...
        foreach ($this->data as $key => $data) {
            ...
            $sheet->setCellValue('H'.$key+10, ' ' . Helper::rupiah($data->tunjangan_operasional));
            $totalTOperasional += $data->tunjangan_operasional;
            ...
            $sheet->setCellValue('K'.$key+10, ' ' . Helper::rupiah($data->tunjangan_lain_lain));
            $totalTLainlain += $data->tunjangan_lain_lain;
            $sheet->setCellValue('L'.$key+10, ' ' . Helper::rupiah($data->lembur));
            $totalLembur += $data->lembur;
            $sheet->setCellValue('M'.$key+10, ' ' . Helper::rupiah($data->reward));
            $totalReward += $data->reward;
            $sheet->setCellValue('N'.$key+10, ' ' . Helper::rupiah($data->infaq));
            $totalInfaq += $data->infaq;
            $sheet->setCellValue('O'.$key+10, ' ' . Helper::rupiah($data->cicilan));
            $totalCicilan += $data->cicilan;
            $sheet->setCellValue('P'.$key+10, ' ' . Helper::rupiah($data->take_home));
        ...
        $sheet->setCellValue('H'.$key+11, ' ' . Helper::rupiah($totalTOperasional ));
        $sheet->setCellValue('I'.$key+11, ' ' . Helper::rupiah($totalTKK ));
        $sheet->setCellValue('J'.$key+11, ' ' . Helper::rupiah($totalTMasaKerja));
        $sheet->setCellValue('K'.$key+11, ' ' . Helper::rupiah($totalTLainlain));
        $sheet->setCellValue('L'.$key+11, ' ' . Helper::rupiah($totalLembur));
        $sheet->setCellValue('M'.$key+11, ' ' . Helper::rupiah($totalReward));
        $sheet->setCellValue('N'.$key+11, ' ' . Helper::rupiah($totalInfaq));
        $sheet->setCellValue('O'.$key+11, ' ' . Helper::rupiah($totalCicilan));
        $sheet->setCellValue('P'.$key+11, ' ' . Helper::rupiah($totalTakeHome));
        $sheet->getRowDimension($key+11)->setRowHeight(20);
        $sheet->getStyle('A' . ($key+11) . ':P' . ($key+11))->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB(Color::COLOR_YELLOW);
        $sheet->getStyle('A10:P'.$key+11)->applyFromArray($styleArray);
        $sheet->getStyle('A10:P'.$key+11)->getAlignment()->setIndent(4);
        $sheet->getStyle('A10:P'.$key+11)->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
    }
}
