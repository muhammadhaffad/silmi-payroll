...
class ReportService
{
    ...
    public function makeReport()
    {
        $startDate = Carbon::now()->subMonth()->format('Y-m-29');
        $endDate = Carbon::now()->format('Y-m-28');
        $reports = Devision::with(['employees' => function ($query) use ($startDate, $endDate) {
            $query->withSum(['attendanceLogs' => function($q) use($startDate, $endDate) {
        ...
        if ($count > 0) {
            $report = Report::query()->where('tanggal', date('Y-m-01'))->forceDelete();
        }
        foreach ($reports as $devision) {
